layui.use(['layer', 'form', 'element', 'jquery', 'dialog'], function() {
	...
	var element = topElement = layui.element;
	var form = layui.form;
	var $ = topJQuery= layui.jquery;
...
function openTab(id,text,url) {
    ...
    var mainLayout = topJQuery('#main-layout');
    var isActive = topJQuery('.main-layout-tab .layui-tab-title').find("li[lay-id=" + id + "]");
    if(isActive.length > 0) {
        //切换到选项卡
        topElement.tabChange('tab', id);
    } else {
        topElement.tabAdd('tab', {
            title: text,
            content: '<iframe src="' + url + '" name="iframe' + id + '" class="iframe" framborder="0" data-id="' + id + '" scrolling="auto" width="100%"  height="100%"></iframe>',
            id: id
        });
